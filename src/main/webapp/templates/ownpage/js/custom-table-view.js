$(document).ready(function(){$projectile.ready(function(){
	...
	_filerOpts.templates.itemAppended = '<div class="table-row files-item" data-file-id="{{fi-fId}}" data-file-revisionId="{{fi-rId}}" data-file-type="{{fi-type}}" data-file-extension="{{fi-extension}}" data-file-name="{{fi-name}}" data-file-user="{{fi-createdByName}}" data-file-size="{{fi-size}}" data-file-date="{{fi-created}}" data-file-orderKey="{{fi-orderKey}}">\
											...
											<div class="table-col"><a href="{{fi-file}}" target="_blank" class="item-download-action files-item-title ns-underline" title="{{fi-name}}">{{fi-icon}} <span>{{fi-name | limitTo: 45}}</span></a></div>\
											<div class="table-col">{{fi-size2}}</div>\
											<div class="table-col">{{fi-date}}</div>\
											<div class="table-col">{{fi-createdByName}}</div>\
											<div class="table-col file-comment">{{fi-comment}}</div>\
											<div class="table-col files-item-actions">\
												<ul class="list-inline">\
		   ...
		   {}, 
		   {
			   ...
			   onClick: function(p,e){
					var id = p.closest(".files-item").attr("data-file-orderKey"),
                        ...
                        eXss = $projectile._config.escapeXss;
				   ...
				   modal({
                       ...
                       + "<p>" + $projectile.dateFormat(data.lockTime, true) + "</p>"
                       + "<b>" + $projectile.captions.tComment +":</b><br>"
                       + "<p>" + eXss(data.lockComment) + "</p>",
					   center: false,
					   size: "small",
